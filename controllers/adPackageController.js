const listPackage = require("../models/adPackage");
...
const allPackages = async (req) => {
  ...
  const apiFeatures = new APIFeatures(
    formatedProperty(listPackage.find(params)),
    req.query
  )
    .search()
    .filter()
...
exports.updatePackage = catchAsyncError(async (req, res, next) => {
  ...
  row = await formatedProperty(
    listPackage.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    })
  );
...
exports.deletePackage = catchAsyncError(async (req, res, next) => {
  ...
  await listPackage.deleteOne();

  res.status(200).json({
    success: true,
    message: "Delete listPackage",
  });
});
